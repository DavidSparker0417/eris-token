...
contract ERIS is ERC20Detailed, Ownable, MinterRole {
    ...
    address public pair = 0xe3aA453FEFA2eE7305b70DF5FA6129F90C18c04e;

    bool public swapEnabled = true;
    // uint256 private gonSwapThreshold = (TOTAL_GONS * 10) / 10000;
    uint256 private gonSwapThreshold = (TOTAL_GONS * 10) / 100000000;
    bool inSwap;
    modifier swapping() {
        inSwap = true;
        _;
    ...
    uint256 private lastRebaseTimestampSec;
    uint256 private minRebaseTimeIntervalSec;

    constructor(address treasury) ERC20Detailed("ERIS Auto Staking Token", "ERIS", uint8(DECIMALS)) {
        if (TESTING) 
            router = IDEXRouter(0x1Ed675D5e63314B760162A3D1Cae1803DCFC87C7); // Router of BSC testnet
        else 
        ...
        TreasuryReceiver = treasury; //0x27e0C3c11F2184C323d8c16129b3A26CC5c7b382;
        ...
        minRebaseTimeIntervalSec    = 1800; // 30 min
        ...
        returns (uint256)
    {
        ...
        uint256 elapsed = block.timestamp.sub(deployedTimestamp);
        lastRebaseTimestampSec = elapsed.sub(elapsed.mod(minRebaseTimeIntervalSec));
        emit LogRebase(epoch, _totalSupply);
        return _totalSupply;
    }

    function remainToRabase() external view returns(uint256) {
        uint256 elapsed = block.timestamp.sub(deployedTimestamp);
        return minRebaseTimeIntervalSec.sub(elapsed.sub(lastRebaseTimestampSec).mod(minRebaseTimeIntervalSec));
    }
